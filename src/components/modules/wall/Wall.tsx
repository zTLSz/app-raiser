...
const Wall: React.FC<WallTypes> = (props) => {
  ...
  if (posts.length > 0) {
    postItems = posts.map((post, i) => (
      <WallPost key={i} post={post} user={user} author={author} name={name} />
    ));
  }
  ...
  return (
    <>
      ...
      <AddWallPost>
        ...
        {walldata.isError ? "Что-то пошло не так!" : ""}
        {!walldata.isLoading && !walldata.isError ? (
          <Button
            type="dashed"
            block
            onClick={() =>
              dispatch(
                requestGetWallPosts(user, author, posts[posts.length - 1].date)
              )
            }
          >
            Еще
