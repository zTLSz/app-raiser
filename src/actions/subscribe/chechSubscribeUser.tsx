...
export const requestCheckSubscribeUser =  (subscribingUserId: number, subscribeTargetId: number) => {
  return {
    type: CHECK_SUBSCRIBE_USER_REQUEST,
    payload: { subscribingUserId: subscribingUserId, subscribeTargetId: subscribeTargetId }
  };
};
...
export const checkSubscribeUserError = (error: { code: string, message: string }) => {
  return {
...
export function* sagaCheckSubscribeUser(action: { payload: CheckSubscribeUserTypes, type: string }) {
    ...
    try {
      yield call(() => requestCheckSubscribeUser(subscribingUserId, subscribeTargetId))
...
async function checkSubscribe(payload: CheckSubscribeUserTypes) {
  ...
  const likeinfo = await db.collection("userinfo")
                        .doc(`${subscribeTargetId}`).collection("followers")
                        .doc(`${subscribingUserId}`).get()
